...
setModules(modules)
 ...
 setMsg((prev)=>[...prev,{role:'ai',msg:realdata.answer}])
  ...
  return (
    <div className="sm:flex w-full relative sweep sm:overflow-y-hidden min-h-screen sm:h-screen sweep">
      {
...
<HiMiniSparkles className=" text-2xl inline"/>
        ...
        <div className=" bg-white/40 rounded-2xl p-3 text- text-white my-4   opacity-0 group-hover:opacity-100 left-4 transition">Chat with SkillDex AI</div>
        </div>
        
      }
      <Nav/>
      <div className="sm:w-[35%] w-full overflow-y-auto h-screen  rounded-xl flex flex-col py-24   gap-y-4 sm:mt-24  ">
        {loading && (
                    ...
                    <div
                      className={`flex w-full justify-between mt-2 ${
                        globeindex == moduleindex
                          ? "bg-slate-800 p-4 text-center rounded-xl"
                          : ""
                      }`}
                      key={globeindex}
                    ...
                    {moduleindex == globeindex && (
                      <div>
                        {real[globeindex].explanation.map((obj, index) => {
                          return (
                            <div
                              className={`bg-blue-500/50 mt-2 p-4 rounded hover:cursor-pointer ${index == click?'bg-violet-500':''}`}
                              key={index}
